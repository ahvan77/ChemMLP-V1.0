...
class Lasso(QDialog):
    ...
    def  init_ui(self): 
       ...
       # self.setGeometry(100, 100, 50, 50)
        self.setWindowTitle('Lasso parameters')
        MyIcon(self)
        grid_layout = QGridLayout()
        self.setLayout(grid_layout)
        
        self.label1 = QLabel('\u03B1 From :', self)
    ...
    def run_svr(self):
        try:
                ...
                split=[]
                C1=[]
                def model1(KF_splits,X,y,alfa):
                    from sklearn.linear_model import Lasso
                    model = Lasso(alpha = alfa) # alpha = 0.0199
                ...
                for i in range(i_from, i_to, i_step):
                        alfa = i/100
                        Test_Data, y_test_orig, Predictions, scores, Test_Data1, Predictions1 = model1(k_fold,X,y,alfa)
                   ...
                   # for j in range(0,5):
                      ...
                      #  plt.scatter(y_test_orig,Predictions2)
                        textfile.write('========================= '+ '\n')
                        textfile.write('Lasso parameters: '+ '\n')
                        textfile.write('Alfa = '+str(alfa)+'\n')
                        textfile.write('========================= '+ '\n')
                        textfile.write('The Accuracy of  Lasso model is: ' + str(100-np.mean(APE)) + '\n')
         ...
         #       self.frame = QFrame(self)      
                ...
                Test_Data, y_test_orig, Predictions, scores, Test_Data1, Predictions1= model1(k_fold,X,y,argmax[1][1])
         ...
         # Set up the main widget and layout
           #     main_widget = QtWidgets.QDialog(self)
                ...
                dialog.setWindowTitle('Lasso Results')
                MyIcon(dialog)
           ...
           #     self.text_edit.append('========================= \n')
              #  argmax = [[0, 2, 5]]  # Replace with your argmax list
                text_widget.setText(text_widget.text()+'Lasso parameters: \n')
            #    self.text_edit.append('SVR parameters: \n')
                text_widget.setText(text_widget.text()+'\u03B1 = '+str(argmax[1][1])+'\n')
          #      self.text_edit.append('C = 2^'+str(argmax[0][1])+ '   epsilon= '+ str((argmax[0][2])/14) + '\n')
                text_widget.setText(text_widget.text()+'========================= \n')     
                #self.text_edit.append('========================= \n')
                text_widget.setText(text_widget.text()+'The Accuracy of Lasso model is: ' + str(100-np.mean(APE)) + '\n')
          ...
          #      self.setLayout(main_layout)
               ...
               # self.ax = self.figure.add_subplot(111)
                ...
                textfile = open(self.pixmappath + 'LASSO_'+'Output.txt', 'w')
                df1[target+'_prediction']=Predictions1
                df1.to_csv(self.pixmappath + 'LASSO_Output.csv' , index=False, lineterminator='\n')
                textfile.close()                
        ...
        #        df1[target+'_prediction'] = Predictions1
         #       df1.to_csv('SVR_Output_test.csv', index=False, line_terminator='\n')
