...
class RFRM(QDialog):
    ...
    def  init_ui(self):   
     ...
     #   self.setGeometry(100, 100, 50, 50)
        self.setWindowTitle('RFR parameters')
        MyIcon(self)
        grid_layout = QGridLayout()
        self.setLayout(grid_layout)
        
        self.label1 = QLabel('max_depth (2) :', self)
    ...
    def run_svr(self):
        try:
                ...
                split=[]
                C1=[]
                def model1(md,KF_splits,X,y,X_test1):
                    from sklearn.ensemble import RandomForestRegressor        
                    model = RandomForestRegressor(max_depth=int(md), random_state=0)
             ...
             #   self.root= Toplevel()
                ...
                Test_Data, y_test_orig, Predictions, scores, Test_Data1, Predictions1 = model1(md,k_fold,X,y,X_test1)                        
                APE=100*(abs(y_test_orig-Predictions)/y_test_orig)
                textfile.write('========================= '+ '\n')
                textfile.write('The Accuracy of  Random Forest Regressor (RFR) model is: ' + str(100-np.mean(APE)) + '\n')
                ...
                dialog.setWindowTitle('Random Forest Regressor (RFR) model Results')
                MyIcon(dialog)
                ...
                text_widget.setText(text_widget.text()+'The Accuracy of RFR model is: ' + str(100-np.mean(APE)) + '\n')
                ...
                textfile = open(self.pixmappath + 'RFR_'+'Output.txt', 'w')
                df1[target+'_prediction']=Predictions1
                df1.to_csv(self.pixmappath + 'RFR_Output.csv', index=False, line_terminator='\n')
                textfile.close()                
