...
class GAN_3D(QDialog):
    ...
    def  init_ui(self):
      ...
      #  self.setGeometry(200, 100, 50, 50)
        ...
        self.labelO1pt = QLabel('Loss Function ' , self) 
        self.grid_layout.addWidget(self.labelO1pt, 2, 3)
        self.combo_box01 = QComboBox()
        self.combo_box01.addItems(['mean_squared_error', 'mean_absolute_error', 'binary_crossentropy', 'categorical_crossentropy', 'sparse_categorical_crossentropy'])
        self.grid_layout.addWidget(self.combo_box01, 2, 4)
        ...
        try:
                           ...
                           #         X = np.concatenate((X, y), axis=1)
                            ...
                            # Storing the fit object for later reference
                              ...
                              #  PredictorScalerFit1=PredictorScaler.fit(X_test1)
                                    ...
                                    for batch_size_trial in batch_size_list:
                                        for epochs_trial in epoch_list:
                                          ...
                                          #  print(y_train, y_test)
                                            ...
                                            model.compile(loss=self.combo_box01.currentText(), optimizer=self.combo_box1.currentText())

                                            model.fit(X_train, y_train ,batch_size = batch_size_trial, epochs = epochs_trial, verbose=0)
                                
                                            y_test_trial=y_test #TargetVarScalerFit.inverse_transform(y_test)

                                
                                            print("Shape of y_test_trial:", y_test_trial.shape)
                                            print("Shape of X_test:", X_test.shape)

                                            MAPE = np.mean(100 * (np.abs(np.squeeze(y_test_trial)-PredictorScalerFit.inverse_transform(model.predict(X_test)))/y_test_trial))
                           ...
                           #     Predictions=TargetVarScalerFit.inverse_transform(Predictions)
                                ...
                                y_test_orig = y_test #TargetVarScalerFit.inverse_transform(y_test)
                                ...
                                def GSCV(X_train, y_train, X_test, y_test):
                                    ...
                                    def make_regression_ann(Optimizer_trial):
                                     ...
                                     ##   model.add(tf.keras.layers.Dense(1, kernel_initializer='normal'))
                                        model.compile(loss=self.combo_box01.currentText(), optimizer=Optimizer_trial)
                                        return model
